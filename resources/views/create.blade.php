        ...
        <form action="{{ route('produk.store') }}" method="POST">
            ...
            <div class="mb-3 flex flex-col gap-1">
                ...
                <textarea name="keterangan" id="keterangan" rows="5"
                    class="text-black px-1 border-2 rounded focus:outline-none focus:ring-2 focus:ring-blue-400 @error('keterangan') border-red-500 @enderror">{{ old('keterangan') }}</textarea>
                @error('keterangan')
                    <small class="text-red-500">
                        {{ $message }}
                    </small>
